...
class DBLP:
    ...
    def __open_dblp_file(self, dblp_path:str)->etree._Element:
        ...
        print("Trying to open:", dblp_path)
...
# code en mémoire en bas
        try:
            ...
            print(dblp_path)
            context = etree.iterparse(dblp_path, events=('start', 'end'), load_dtd=True)
            _, root = next(context)
...
#        root = self.__open_dblp_file(dblp_path)
    ...
    # Ouvrir le fichier XML
                ...
                with open(dblp_path, 'rb') as f:
                    ...
                    for event, element in etree.iterparse(f, events=('start', 'end'), load_dtd=True):
                        ...
                        if event == 'end':
                            if element.tag in self.all_elements:
                                #attrib_values = self.__extract_features(element, features_to_extract, include_key_and_mdate)
                                attrib_values=[element.tag, element.text]
                                file.write(json.dumps(attrib_values) + '\n')
            ...
            for element in root:
                if element.tag in self.all_elements:
                    attrib_values = self.__extract_features(element, features_to_extract, include_key_and_mdate)
                    dataframe = dataframe._append(attrib_values, ignore_index=True)
