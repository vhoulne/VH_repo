...
#test
        ...
        while c1<3 and c2<3:
            while c1<3:
                if getCompteur() > 0 :              
                    ...
                    time.sleep(0.2)
                    if pair==1:
                        dist= -0.43*hauteur+109
                        temps=1.6*60/dist + 0.2
                        Motor.MotorRun(0, 'forward', 90)
                ...
                Motor.MotorRun(0, 'forward', 0)
                Motor.MotorRun(1, 'forward', 0)
                time.sleep(0.3)                               ###il fait 3 * le process vaire la balise impaire
        ...
        Motor.MotorRun(0, 'forward', 0)
        Motor.MotorRun(1, 'forward', 0)   #quand il voit une balise il se stop
...
def commencer():
    ...
    while compteur_num <4:
        ...
        recherchebalisecercle(num_sortie)  
    r = requests.get("http://robotpi-22:8080/slave_end")
    print(r.status_code)  
...
thread_reco.start()
requests.get("127.0.0.1:8000/commencer")
thrd1.join()
thread_reco.join()
